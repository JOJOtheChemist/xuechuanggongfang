<script>
import { writeStorageSync } from './utils/storage-bridge'
...
export default {
	...
	onLaunch(options) {
		...
		// 处理通过团队邀请二维码进入小程序的场景
		void this.handleInviteScene(options)
	},
	onShow(options) {
	...
	methods: {
		...
		async handleInviteScene(options) {
			if (!options) return
			...
			let inviterId = ''
			let handled = false
			let pendingTeamInvite = null
			let pendingBusinessInvite = null
			...
			if (scene.indexOf('i=') === 0) {
				...
				if (uid) {
					pendingTeamInvite = { inviter: uid, type: 'team_invite', timestamp: Date.now(), source: 'app_launch' }
					inviterId = uid
					handled = true
				} else {
				...
				if (data.inviter) {
					...
					pendingTeamInvite = Object.assign({
						timestamp: Date.now(),
						source: 'app_launch'
					}, data)
					handled = true
			...
			if (scene.indexOf('b=') !== -1) {
				...
				if (bizData.inviter) {
					...
					bizData.timestamp = Date.now()
					bizData.source = 'app_launch'
					pendingBusinessInvite = bizData
					handled = true
				}
			}
			...
			if (scene.indexOf('c=') !== -1) {
				...
				handled = true
				// 短码需要异步解析，此处仅放行跳转到 invite-handler 处理
			}

			console.log('[App] scene 解析结果 - inviterId:', inviterId, 'handled:', handled)
			...
			try {
				// 先等 storage 桥稳定，再落盘，避免启动过早访问失败
				if (inviterId) {
					await writeStorageSync('pending_inviter_id', inviterId)
				}
				if (pendingTeamInvite) {
					await writeStorageSync('pending_team_invite', pendingTeamInvite)
				}
				if (pendingBusinessInvite) {
					await writeStorageSync('pending_business_invite', pendingBusinessInvite)
				}
			} catch (error) {
				console.warn('[App] 邀请信息缓存写入失败，继续依赖 URL 参数:', error)
			}
