...
function createPromptEntry(label, routeUrl, extras = {}) {
	const text = String(label || '').trim()
	return {
		label: text,
		action: text,
		routeUrl: String(routeUrl || '').trim(),
		...extras
	}
}

function createPromptEntries(labels = [], routeUrl = '', extras = {}) {
	return labels.map((label) => createPromptEntry(label, routeUrl, extras))
}

const XIAOCHUNLU_TOPIC_PROMPTS = {
	article: {
		guessPrompts: createPromptEntries([
			'文章板块里现在有什么可以先看？',
			'如果我是新手，先看哪些文章更合适？',
			'你先按我的情况推荐几篇文章',
			'我时间不多，先看哪类资料最值？',
		...
		suggestionPrompts: createPromptEntries([
			'先推荐文章',
			'新手先看哪些',
			'时间少看什么',
			'先排个优先级'
...
const XIAOCHUNLU_AGENT_UI_CONFIG = {
	assistantName: '小春鹿',
	...
	quickPrompts: [
		createPromptEntry('先看文章', '', { tone: 'primary' }),
		createPromptEntry('先看业务入口', '', { tone: 'outline' })
	],
	introQuickPrompts: [
...
const AGENT_UI_CONFIGS = {
	...
	'xiaochunlu-ai-v3': XIAOCHUNLU_AGENT_UI_CONFIG,
	'xiaochunlu-ai-v2': XIAOCHUNLU_AGENT_UI_CONFIG,
	'xiaochunlu-ai-v5': XIAOCHUNLU_AGENT_UI_CONFIG,
}
